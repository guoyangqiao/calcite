...
public class HepPlannerTest extends RelOptTestBase {
    ...
    public void testRuleInstance() {
        ...
        programBuilder.addRuleInstance(new AccRule());

        HepPlanner planner =
                new HepPlanner(
                        programBuilder.build());
        final String select_name_from_dept = "select * from  dept ";
//        final String select_name_from_dept = "select dept.name from   (select * from  emp where fname <> '23')  as emp, dept  where dept.name  = emp.fname ";
        final RelRoot relRoot = tester.convertSqlToRel(select_name_from_dept);
    ...
    static class AccRule extends RelOptRule {

        public AccRule() {
            super(RelOptRule.operand(TableScan.class, RelOptRule.any()));
        }

        @Override
        public boolean matches(RelOptRuleCall call) {
            final TableScan rel = call.rel(0);
            final List<String> qualifiedName = rel.getTable().getQualifiedName();
            return !qualifiedName.get(qualifiedName.size() - 1).startsWith("foooo");
        }
        ...
        public void onMatch(RelOptRuleCall call) {
            ...
            List<String> strings = new ArrayList<>(qualifiedName);
            strings.set(strings.size() - 1, "fooooo" + System.currentTimeMillis());
            final Table unwrap = table.unwrap(Table.class);
            final RelOptTableImpl relOptTable1 = RelOptTableImpl.create(table.getRelOptSchema(), table.getRowType(), unwrap, ImmutableList.copyOf(strings));
            final LogicalTableScan logicalTableScan = new LogicalTableScan(rel.getCluster(), rel.getTraitSet(), relOptTable1);
            call.transformTo(logicalTableScan);
        }
    }
